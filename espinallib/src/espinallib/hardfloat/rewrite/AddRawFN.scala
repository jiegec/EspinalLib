...
class AddRawFN(
  ...
  val modNatAlignDist: UInt =
    Mux(sDiffExps < S(0), io.b.sExp - io.a.sExp, sDiffExps)
      .asUInt(0, alignDistWidth - 1 bits)
  val isMaxAlign: Bool =
    (sDiffExps >> alignDistWidth) =/= S(0) &&
      ((sDiffExps >> alignDistWidth) =/= S(-1) || sDiffExps.asBits(
        0,
        alignDistWidth - 1 bits
      ) === B(0))
  val alignDist: UInt =
    Mux(isMaxAlign, U((BigInt(1) << alignDistWidth) - 1), modNatAlignDist)
  ...
  val close_sigSum: UInt =
    Mux(close_sSigSum < S(0), -close_sSigSum, close_sSigSum)
      .asUInt(0, sigWidth + 1 bits)
  ...
  val close_nearNormDist: UInt =
    (close_normDistReduced2 << 1)(0, alignDistWidth - 1 bits)
  val close_sigOut: UInt =
    ((close_sigSum << close_nearNormDist) << 1)(0, sigWidth + 2 bits)
  val close_totalCancellation: Bool =
    !(close_sigOut((sigWidth + 2) downto (sigWidth + 1)).orR)
  val close_notTotalCancellation_signOut: Bool =
    io.a.sign ^ (close_sSigSum < S(0))
  /*------------------------------------------------------------------------
   *------------------------------------------------------------------------*/
  val far_signOut: Bool = Mux(sDiffExps < S(0), effSignB, io.a.sign)
  val far_sigLarger: UInt =
    Mux(sDiffExps < S(0), io.b.sig, io.a.sig)(sigWidth - 1 downto 0)
  val far_sigSmaller: UInt =
    Mux(sDiffExps < S(0), io.a.sig, io.b.sig)(sigWidth - 1 downto 0)
  ...
  val far_roundExtraMask: UInt =
    LowMask(alignDist(alignDistWidth - 1 downto 2), (sigWidth + 5) / 4, 0)
  ...
  val far_negAlignedSigSmaller: Bits =
    Mux(far_subMags, Cat(U(1), ~far_alignedSigSmaller), far_alignedSigSmaller)
  val far_sigSum: UInt =
    (far_sigLarger << 3) + far_negAlignedSigSmaller.asUInt + far_subMags.asUInt
  val far_sigOut: UInt =
    Mux(far_subMags, far_sigSum, (far_sigSum >> 1) | far_sigSum(0 downto 0))(
      sigWidth + 2 downto 0
    )
  /*------------------------------------------------------------------------
   *------------------------------------------------------------------------*/
